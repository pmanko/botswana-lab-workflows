...
async function getIpmsCode(q: string, c = '') {
  try {
    ...
    let mappingIndex = ipmsMappings.findIndex(
      (x) => x.map_type == 'BROADER-THAN' && x.to_concept_code == c,
    )

    // Fall back to "SAME AS"
    if (mappingIndex < 0) {
      mappingIndex = ipmsMappings.findIndex(
        (x) => x.map_type == 'SAME-AS' && x.to_concept_code == c,
      )
    }
    ...
    if (mappingIndex >= 0) {
      ...
      if (ipmsCodingInfo) {
        ipmsMnemonic = ipmsCodingInfo.names.find((x) => x.name_type == 'Short').name
        hl7Flag =
...
async function getOclMapping(queryString: string): Promise<any[]> {
  const options = { timeout: { request: config.get('bwConfig:requestTimeout') | 1000 } }
...
async function getOclConcept(conceptCode: string): Promise<any> {
  const options = {
    timeout: {
      request: config.get('bwConfig:requestTimeout') | 1000
    }
  }

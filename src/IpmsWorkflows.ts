...
import { getTaskStatus, setTaskStatus } from './helpers.ts'
import { hl7Sender } from './lib/hl7MllpSender.ts'
import Hl7WorkflowsBw from './hl7Workflows.ts'
import got from 'got'
import { translateCoding } from './terminologyWorkflows.ts'
import {
  BundleTypeKind,
...
import { saveBundle } from './lib/hapi/lab.ts'
import { hapiGet } from './lib/hapi/helpers.ts'
...
export async function sendOrmToIpms(bundles): Promise<R4.IBundle> {
  ...
  try {
    ...
    const pindex = labBundle.entry!.findIndex((entry) => {
    ...
    for (const sr of srBundle.entry) {
      ...
      if (ormMessage && ormMessage != '') {
        const result = await hl7Sender.send(ormMessage, targetIp, targetPort)
 ...
 */
export async function handleOruFromIpms(message): Promise<R4.IBundle> {
  ...
  try {
    ...
    if (translatedBundle && translatedBundle.entry) {
      ...
      let dr: IDiagnosticReport = <IDiagnosticReport>(
        translatedBundle.entry.find((e) => e.resource && e.resource.resourceType == 'DiagnosticReport')!.resource!
      )
      ...
      const { omang, bcn, ppn, patOptions }: any = processIpmsPatient(patient)

      logger.info(`omang: ${omang}, bcn: ${bcn}, ppn: ${ppn}, patOptions: ${JSON.stringify(patOptions)}`)
      ...
      */
      ...
      const labOrderId = dr.identifier && dr.identifier.length > 0 ? dr.identifier.find((i) => i.system == config.get('bwConfig:labOrderSystemUrl')) : undefined
      const labOrderMrn = dr.identifier && dr.identifier.length > 0 ? dr.identifier.find((i) => i.system == config.get('bwConfig:mrnSystemUrl')) : undefined
      
      logger.info('Lab Order ID: ' + JSON.stringify(labOrderId) + ' MRN: ' + JSON.stringify(labOrderMrn))
      ...
      if(serviceRequestBundle && serviceRequestBundle.entry && serviceRequestBundle.entry.length > 0) {  
        ...
        taskPatient = <IPatient>(
          serviceRequestBundle.entry.find((e) => e.resource && e.resource.resourceType == 'Patient')!
            .resource!
        )

        task = <ITask>(
          serviceRequestBundle.entry.find((e) => e.resource && e.resource.resourceType == 'Task')!.resource!
        )
...
function processIpmsPatient(patient: R4.IPatient) {

...
import { WorkflowHandler, topicList } from './workflowHandler.ts'
...
export default class Hl7WorkflowsBw {
  ...
  static async retryOperation(func: () => any, maxRetries: number, delay: number, errorCheck: (result) => boolean, payloadForDMQ) {
    let attempts = 0;
    let result;

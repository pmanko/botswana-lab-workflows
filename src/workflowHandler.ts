...
import { KafkaProducerUtil } from './lib/kafkaProducerUtil.ts'
import logger from './lib/logger.ts'
import { KafkaConfig, ProducerRecord } from 'kafkajs'
import { logLevel } from 'kafkajs'
import { handleAdtFromIpms, handleOruFromIpms, sendAdtToIpms, sendOrmToIpms } from './IpmsWorkflows.ts'
import { mapConcepts } from './terminologyWorkflows.ts'
import { mapLocations } from './locationWorkflows.ts'
import { saveIpmsPatient, updateCrPatient } from './patientIdentityWorkflows.ts'
...
export class WorkflowHandler {
  ...
  static async executeTopicWorkflow(topic: string, val): Promise<WorkflowResult> {
    let response
    let enrichedBundle
    let origBundle
    let hl7Message
    ...
    try {
      switch (topic) {
        ...
        case topicList.HANDLE_ORU_FROM_IPMS: {
          ...
          logger.info(`Response from handleOruFromIpms: ${JSON.stringify(respose)}`)
          break
        }
   ...
   */
  public static async sendPayload(payload, topic: string) {
